...
public:
    ...
    static string EncryptMessage(string Text, short Key) {
        for (short i = 0; i <= Text.length(); i++) {
            Text[i] = char((int)Text[i] + Key);
        }
        return Text;
    }
    static string DecryptMessage(string Text, short Key) {
        for (short i = 0; i <= Text.length(); i++) {
            Text[i] = char((int)Text[i] - Key);
        }
        return Text;
    ...
    static string ReadPassword() {
        string Password;
        cout << "Enter a Password From 3 letters to Guess: \n";
        ...
        for (short i = 65; i <= 90; i++) {
            for (short j = 65; j <= 90; j++) {
                for (short k = 65; k <= 90; k++) {
                    ...
                    Word = Word + char(j);
                    Word = Word + char(k);
                    cout << "Tiral [" << Counter << "] : " << Word << endl;
                    if (Word == Password) {
                        cout << "The Password is : " << Word << endl;
                        cout << "Found After Tiral(s) " << Counter << endl;
                        return true;
                    }
                    Word = "";
                }
